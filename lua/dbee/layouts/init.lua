...
---@field open fun(self: Layout, opts: { reset: boolean}) function to open ui.
...
---@param name string
---@param win integer
function layouts.Default:reset_window_size(name, win)
  if name == "result" then
    vim.api.nvim_win_set_height(win, self.result_height)
  elseif name == "drawer" then
    vim.api.nvim_win_set_width(win, self.drawer_width)
  elseif name == "call_log" then
    vim.api.nvim_win_set_height(win, self.call_log_height)
  end
end

---@package
---@param opts { reset : boolean }
function layouts.Default:open(opts)
  if opts.reset then
    for win in pairs(self.windows) do
      self:reset_window_size(win, self.windows[win])
    end
    return
  end
